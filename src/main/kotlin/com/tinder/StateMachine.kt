...
class StateMachine<STATE : Any, EVENT : Any, SIDE_EFFECT : Any> private constructor(
  ...
  inline fun <reified S : STATE, reified E : EVENT>toModel(): StateMachineModel {
    val events = E::class.nestedClasses.map { it.objectInstance as E }
    val states = S::class.nestedClasses.map { it.objectInstance as S }
    val sideEffects = mutableSetOf<SIDE_EFFECT>()
    val stateModels: List<StateModel> = states.map { state ->
      var stateName = state.toString()
      var eventName: String

      val eventModels = events.mapNotNull { event ->
        val transition = state.getTransition(event)
        if (transition is Transition.Valid) {
          val edge = transition.event
          val fromState = transition.fromState
          val toState = transition.toState
          val sideEffect = transition.sideEffect

          var transitionModel: TransitionModel? = null
          var dontTransition: DontTransition? = null

          if (!sideEffects.contains(sideEffect)) {
            sideEffects.add(sideEffect!!)
          }

          stateName = transition.fromState.toString()
  ...
  inline fun <reified S : STATE, reified E : EVENT>dotDiagram(): String {
    val events = E::class.nestedClasses.map { it.objectInstance as E }
    val states = S::class.nestedClasses.map { it.objectInstance as S }
    val sb = StringBuilder()
    sb.appendln("digraph StateMachine {")
    val sideEffects = mutableSetOf<SIDE_EFFECT>()
    states.forEach { state ->
      events.forEach { event ->
        ...
        if (transition is Transition.Valid) {
          ...
          if (fromState != toState) {
            sb.appendln("\t\"$fromState\" -> \"$toState\" [label=\"$edge\"];")
          } else {
            if (!sideEffects.contains(sideEffect)) {
              sb.appendln("\t\"$sideEffect\" [ shape=\"box\"];")
              sideEffects.add(sideEffect!!)
            }
            sb.appendln("\t\"$fromState\" -> \"$sideEffect\" [shape=\"box\",label=\"$edge\"];")
          }
        }
      }
    }
  ...
  fun STATE.getTransition(event: EVENT): Transition<STATE, EVENT, SIDE_EFFECT> {
    val transitions = getDefinition()?.transitions
    for ((eventMatcher, createTransitionTo) in transitions.orEmpty()) {
